class Api::V1::OrdersController < ApiController
  ...
  api :POST, '/v1/refund_order', 'Change status of order to refund and send back BCH to buyer'
  param :auth_token, String, desc: "Authentication Token User", required: true
  param :order_id, String, desc: "Id of order"
  param :amount, String, desc: "Amount of BCH will be send back to buyer"
  param :buyer_address, String, desc: "BCH address of buyer"
  def refund_order
    current_user = User.where(authentication_token: params[:auth_token]).first
    order = Order.refund_order(current_user ,params[:order_id], params[:amount], params[:buyer_address].gsub(/\n/, ''))
    render json: { status: "This order already cancel and vendor will be sent back BCH to buyer" }, status: :success
  end

  api :GET, '/v1/form_refund', 'show information BCH of user'
  param :auth_token, String, desc: "Authentication Token User", required: true
  def account
    current_user = User.where(authentication_token: params[:auth_token]).first
  ...
  api :GET, '/v1/create_new_bitcoin_address', 'generate new address of BCH'
  param :auth_token, String, desc: "Authentication Token User", required: true
  ...
  api :POST, '/v1/withdraw_funds', 'Withdraw BCH of user'
  param :auth_token, String, desc: "Authentication Token User", required: true
  param :withdraw_password, String, desc: "withdraw password of user"
  param :amount, String, desc: "Amount of BCH will be withdraw"
  param :bitcoin_address, String, desc: "BCH address will receive the BCH"
  def withdraw_funds
    current_user = User.where(authentication_token: params[:auth_token]).first
    ...
    if old_withdraw_password == params[:withdraw_password]
      if params[:amount].to_f <= escrow
        ...
        render json: { status: "Sorry your BCH not enough for this transaction" }, status: :unprocessable_entity
      end
    else
  ...
  api :POST, '/v1/transfer_fund', 'Transfer BCH each user'
  param :auth_token, String, desc: "Authentication Token User", required: true
  param :withdraw_password, String, desc: "withdraw password of user"
  param :amount, String, desc: "Amount of BCH will be withdraw"
  param :bitcoin_address, String, desc: "BCH address will receive the BCH"
  def transfer_fund
    ...
    if old_withdraw_password == params["withdraw_password"]
      if params[:amount].to_f <= bitcoin_balance
        ...
        render json: { status: "Congratulation your transfer BCH success sent to #{receive}" }, status: :success
      else
        render json: { status: "Sorry your BCH is not enought to make a transfer process" }, status: :unprocessable_entity
      end
    else
      render json: { status: "Sorry your withdraw password not match. Please input the correct password" }, status: :unprocessable_entity
    end
