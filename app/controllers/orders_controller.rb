...
class OrdersController < InheritedResources::Base
  ...
  def pay
    if current_user.valid_password?(params[:password])
      ...
      if (balance - shopping_cart.orders.map(&:total_payment).sum.to_f.round(6)) < 0
        redirect_to pay_order_path(shopping_cart_id: params[:shopping_cart]), alert: "Sorry you BCH not enought to pay this order"
      else
    ...
    if current_user.valid_password?(params["password"])
      if params[:amount].to_f <= @bitcoin_balance.to_f
        ...
        if tx_id.present?
          ...
          redirect_to bitcoin_account_url, notice: "Congratulation your transfer BCH success sent to #{receive}"
        else
          redirect_to bitcoin_account_url, alert: "Sorry the transaction is failed"
        end
      else
        redirect_to bitcoin_account_url, alert: "Sorry your BCH is not enough to make a transfer process"
      end
    else
      redirect_to bitcoin_account_url, alert: "Sorry your transaction password not match. Please input the correct password"
    end
  end
  ...
  def cancel
    ...
    session.delete(:shopping_cart_id)
    if order
      msg = "This order already cancel and vendor will be sent back BCH to buyer"
    else
