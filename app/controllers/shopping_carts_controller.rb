class ShoppingCartsController < ApplicationController
  ...
  def create
    ...
    shipping = ShippingOption.find(params[:shipping])

    # if bitcoin_balance == nil
    #   balance = 0.0
    # else
    ...
    # if balance.to_f < (total_payment.round(6) + @item.price.to_f.round(6))
    #   redirect_to item_detail_path(random_string: @item.random_string), alert: "Sorry you balance is not enought for #{@item.name}"
    # else
  ...
  def show 
    ...
    if @total_payment > @bitcoin_balance.to_f 
      flash[:alert] = "Your account has insufficient BCH, please deposit more on Account page"
    end
    ...
    hash.each do |key, value|
      ...
      if item_quantity > value[3].to_i || item.unlimited
        ...
        if (item.currency.eql?"Bitcoin") || (item.currency.eql?"BCH")
          ...
          @price_BCH = item.price.to_f / group.first['rate'].to_f
          price = @price_BCH * value[3].to_i
        end

        shipping = ShippingOption.find(value[2])

        unless shipping.currency =~ /BCH|Bitcoin/
          ...
          @shipping = shipping.price.to_f / group_local.first['rate'].to_f rescue 0
        end
  ...
  def validate_in_update_cart
    ...
    if @new_price > bitcoin_balance.to_f 
      redirect_to shopping_cart_path, alert: "Your account has insufficient BHC, please deposit more on Account page"
    end
  end
  ...
  def get_new_price
    ...
    hash.each do |key, value|
      ...
      if (item.currency.eql?"Bitcoin") || (item.currency.eql?"BCH")
        ...
        group = @rates.select { |element_hash| element_hash["code"].eql?"#{item.currency}" }
        @price_BCH = (item.price.to_f / group.first['rate'].to_f) *  value[1].to_i
        price = @price_BCH 
      end
  ...
  def check_buyer_balance
    ...
    if bitcoin_balance.to_f < total_payment.to_f
      redirect_to item_detail_url(item.random_string), alert: "Your account has insufficient BCH, please deposit more on Account page"
    end
  end
