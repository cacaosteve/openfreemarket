class ShoppingCart < ActiveRecord::Base
  ...
  def pay(params, current_user, rates)
    ...
    tx_id = `bitcoin-cli move #{current_user.username} #{receiver} #{total.to_f.round(6)} 1 '{ "from" => #{current_user.username}, "to" => #{receiver}'` rescue ""
  ...
  def get_shipping_price(shipping_option, rates)
    if shipping_option.currency.eql?("BCH") || shipping_option.currency.eql?("Bitcoin")
      ...
      shipping_price = shipping_option.price.to_f / group_local.first['rate'].to_f rescue 0
    end
    shipping_price
  end
end
